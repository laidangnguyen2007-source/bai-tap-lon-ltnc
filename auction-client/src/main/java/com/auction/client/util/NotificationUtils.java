...
/**
 * Tiện ích hiển thị thông báo "Toast" (thông báo nhỏ tự biến mất) trong JavaFX. Giúp người dùng
 * nhận biết các sự kiện Real-time mà không bị gián đoạn trải nghiệm.
 */
public class NotificationUtils {
    ...
    Platform.runLater(
        () -> {
          ...
          container.setStyle(
              "-fx-background-color: "
                  + bgColor
                  + "; -fx-background-radius: 10; -fx-border-radius: 10; -fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.4), 10, 0, 0, 5);");
          ...
          double height =
              container.getHeight() > 0 ? container.getHeight() : container.prefHeight(300);
          ...
          if (activePopups.size() >= 3) {
            ...
            if (!oldest.getContent().isEmpty()) {
              FadeTransition hideOldest =
                  new FadeTransition(Duration.millis(200), oldest.getContent().get(0));
  ...
  /**
   * Nhuộm màu cho hộp thoại Dialog để đồng bộ với giao diện Dark Theme của ứng dụng. Phương thức
   * này nạp file styles.css vào DialogPane của Dialog.
   *
  ...
  public static void styleDialog(javafx.scene.control.Dialog<?> dialog) {
    ...
    String css =
        NotificationUtils.class.getResource("/com/auction/client/css/styles.css").toExternalForm();
