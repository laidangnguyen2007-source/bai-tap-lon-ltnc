...
/**
 * Utility class chuyên trách việc chuyển đổi JSON sang Java Object (Deserialize). Tuân thủ SRP: Các
 * lớp Network chỉ lo gửi/nhận, lớp này lo parse dữ liệu.
 *
 * <p>Tất cả phương thức là static — không cần khởi tạo đối tượng. Null-safety: tất cả trường số đều
 * ép qua (Number) thay vì (Long) để tránh ClassCastException khi server trả về Integer hay
 * BigDecimal.
 */
public final class JsonMapper {
  ...
  public static User mapToUser(JSONObject json) {
    ...
    return switch (role) {
      case "BIDDER" -> {
        long balance =
            json.get("balance") != null ? ((Number) json.get("balance")).longValue() : 0L;
  ...
  public static Auction mapToAuction(JSONObject json) {
    ...
    Long sellerId =
        json.get("sellerId") != null ? Long.parseLong(json.get("sellerId").toString()) : null;
    long currentPrice =
        json.get("currentPrice") != null ? Long.parseLong(json.get("currentPrice").toString()) : 0L;
    Long currentWinnerId =
        json.get("currentWinnerId") != null
            ? Long.parseLong(json.get("currentWinnerId").toString())
            : null;
    ...
    long minBidStep =
        json.get("minBidStep") != null ? Long.parseLong(json.get("minBidStep").toString()) : 0L;
    ...
    a.setItemName(
        json.get("itemName") != null ? json.get("itemName").toString() : "Sản phẩm #" + itemId);
    a.setItemCategory(
        json.get("itemCategory") != null ? json.get("itemCategory").toString() : "OTHER");
    a.setItemDescription(
        json.get("itemDescription") != null ? json.get("itemDescription").toString() : "");
    a.setItemSpecifics(
        json.get("itemSpecifics") != null ? json.get("itemSpecifics").toString() : "");
    a.setImageBase64(json.get("imageBase64") != null ? json.get("imageBase64").toString() : null);

    // Nhận giá khởi điểm và gán cho auction hiện tại để trả về
    long startingPrice =
        json.get("startingPrice") != null
            ? Long.parseLong(json.get("startingPrice").toString())
            : 0L;
  ...
  public static BidTransaction mapToBid(JSONObject json) {
    ...
    Long auctionId =
        json.get("auctionId") != null ? ((Number) json.get("auctionId")).longValue() : -1L;
    Long bidderId =
        json.get("bidderId") != null ? ((Number) json.get("bidderId")).longValue() : -1L;
  ...
  public static Item mapToItem(JSONObject json) {
    ...
    String desc =
        json.get("description") != null ? (String) json.get("description") : "Đang cập nhật...";
        ...
        switch (cat) {
          case ELECTRONICS ->
              ItemFactory.reconstructElectronics(
                  ...
                  json.get("warrantyMonths") != null
                      ? ((Number) json.get("warrantyMonths")).intValue()
                      : 0,
                  json.get("powerWatts") != null
                      ? ((Number) json.get("powerWatts")).doubleValue()
                      : 0.0);
          case ARTWORK ->
              ItemFactory.reconstructArtwork(
                  ...
                  json.get("yearCreated") != null
                      ? ((Number) json.get("yearCreated")).intValue()
                      : 0,
          ...
          case VEHICLE ->
              ItemFactory.reconstructVehicle(
                  ...
                  json.get("yearManufactured") != null
                      ? ((Number) json.get("yearManufactured")).intValue()
                      : 0,
