...
public class AuctionDetailController {
  ...
  private String formatTimeLeft(Auction a) {
    java.time.Duration d =
        java.time.Duration.between(java.time.LocalDateTime.now(), a.getEndTime());
  ...
  private void handleImageClick(javafx.scene.input.MouseEvent event) {
    ...
    javafx.scene.image.ImageView fullImageView =
        new javafx.scene.image.ImageView(itemImageView.getImage());
    ...
    javafx.scene.control.ScrollPane scrollPane =
        new javafx.scene.control.ScrollPane(imageContainer);
    ...
    javafx.scene.control.Label hintLabel =
        new javafx.scene.control.Label(
            "💡 Giữ Ctrl + Cuộn chuột để Thu/Phóng. Kéo thả chuột để di chuyển ảnh.");
    hintLabel.setStyle(
        "-fx-text-fill: white; -fx-padding: 12px; -fx-background-color: #1a1d29; -fx-font-size: 13px; -fx-alignment: center;");
