...
public class MyWinsController {
  ...
  public void initialize() {
    ...
    finalPriceColumn.setCellValueFactory(
        cellData ->
            new SimpleStringProperty(String.format("%,d", cellData.getValue().getCurrentPrice())));

    endTimeColumn.setCellValueFactory(
        cellData -> new SimpleStringProperty(cellData.getValue().getEndTime().format(DATE_FORMAT)));
  ...
  private void loadWins() {
    ...
    List<Auction> won =
        all.stream()
            ...
            .filter(
                a -> a.getStatus() == AuctionStatus.FINISHED || a.getStatus() == AuctionStatus.PAID)
