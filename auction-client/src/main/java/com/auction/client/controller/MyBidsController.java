...
public class MyBidsController {
  ...
  public void initialize() {
    ...
    bidTimeColumn.setCellValueFactory(
        cellData -> new SimpleStringProperty(cellData.getValue().getBidTime().format(DATE_FORMAT)));
  ...
  private void loadBids() {
    ...
    Map<Long, Auction> auctionMap =
        allAuctions.stream().collect(Collectors.toMap(Auction::getId, a -> a));
    ...
    for (BidTransaction bid : myBids) {
      ...
      rows.add(
          new BidRow(bid.getAuctionId(), itemName, bid.getAmount(), endTime, bid.getTimestamp()));
    }
