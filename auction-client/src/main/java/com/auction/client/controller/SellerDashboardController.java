...
public class SellerDashboardController implements com.auction.client.observer.AuctionObserver {
    ...
    private static final String[] CATEGORY_ENUM_VALUES = {"ELECTRONICS", "ARTWORK", "VEHICLE", "OTHER"};
    ...
    private void renderGrid(List<Auction> auctions) {
        ...
        for (Auction auction : auctions) {
            ...
            Label nameLbl = new Label(auction.getItemName() != null ? auction.getItemName() : "Sản phẩm #" + auction.getItemId());
            ...
            Label statusLbl = new Label(translateStatus(auction.getStatus()) + " | " + translateCategoryToVi(auction.getItemCategory()));
            ...
            Runnable updateTime = () -> {
                ...
                } else if (timeLeft.contains("kết thúc") || timeLeft.contains("thanh toán") || timeLeft.contains("hủy")) {
                    timeLbl.setStyle("-fx-font-size: 14px; -fx-text-fill: #94a3b8;");
                } else {
                    timeLbl.setStyle("-fx-font-size: 14px; -fx-text-fill: #2ecc71; -fx-font-weight: bold;");
                }
            };
            updateTime.run();
    ...
    private void navigateToDetail(Auction selected) {
        ...
        try {
            ...
            FxmlLoader.navigateTo(stage, "auction-detail.fxml", "Online Auction System — Chi Tiết Sản Phẩm");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void showEditModal(Auction auction) {
        ...
        for (int i = 0; i < CATEGORY_ENUM_VALUES.length; i++) {
            if (CATEGORY_ENUM_VALUES[i].equals(enumCat)) { idx = i; break; }
        }
    ...
    private void handleCreateOrUpdateAuction(ActionEvent event) {
        ...
        LocalDateTime startTime = startDatePicker.getValue().atTime(startHourCombo.getValue(), startMinuteCombo.getValue());
        LocalDateTime endTime = endDatePicker.getValue().atTime(endHourCombo.getValue(), endMinuteCombo.getValue());
        ...
        if (editingAuctionId != null) {
            ...
            if (createdId != null && createdId > 0) {
                hideCreateModal(null);
                NotificationUtils.showSuccess((Stage) refreshButton.getScene().getWindow(), "Tạo phiên đấu giá #" + createdId + " thành công!");
                loadMyAuctions();
            } else {
    ...
    private void handleDelete(Auction auction) {
        ...
        if (confirmResult.isPresent() && confirmResult.get() == javafx.scene.control.ButtonType.OK) {
            ...
            if (success) {
                NotificationUtils.showSuccess((Stage) refreshButton.getScene().getWindow(), "Đã xóa phiên đấu giá #" + auction.getId());
                loadMyAuctions();
            } else {
                NotificationUtils.showError((Stage) refreshButton.getScene().getWindow(), "Xóa thất bại. Phiên có thể đã có người đặt giá.");
            }
        }
    }
    ...
    public void onBidUpdated(server.model.entity.BidTransaction bid) {
        Platform.runLater(() -> {
            NotificationUtils.showToast((Stage) refreshButton.getScene().getWindow(), "📣 Giá mới cho #"
    ...
    public void onAuctionStatusChanged(Long auctionId, String newStatus) {
        Platform.runLater(() -> {
            NotificationUtils.showToast((Stage) refreshButton.getScene().getWindow(), "🔔 Phiên #"
                + auctionId + " đã chuyển sang trạng thái: " + newStatus, false);
            loadMyAuctions();
        });
    }
}
