...
/**
 * Controller cho màn hình Quản Lý Ví Admin (admin-wallet.fxml).
 * Cho phép admin xem tất cả ví, điều chỉnh số dư, và xem lịch sử giao dịch.
 */
public class AdminWalletController {
    ...
    public void initialize() {
        ...
        walletTable.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal != null) {
    ...
    private void handleAdjust(ActionEvent event) {
        ...
        new Thread(() -> {
            boolean success = serverService.adminAdjustBalance(userId, amount, adminId, "ADMIN", desc);
            Platform.runLater(() -> {
                if (success) {
                    ...
                    loadAllWallets();
                    loadTransactionsForUser(userId);
                    NotificationUtils.showSuccess((Stage) refreshButton.getScene().getWindow(),
                        "Điều chỉnh số dư cho user #" + userId + " thành công!");
                } else {
                    adjustResultLabel.setText("❌ Điều chỉnh thất bại. Vui lòng kiểm tra lại.");
                    NotificationUtils.showError((Stage) refreshButton.getScene().getWindow(),
                        "Điều chỉnh thất bại.");
                }
            });
        }, "AdminAdjust").start();
    ...
    private void handleRefresh(ActionEvent event) {
        ...
        NotificationUtils.showToast((Stage) refreshButton.getScene().getWindow(), "✅ Đã làm mới", false);
    }
