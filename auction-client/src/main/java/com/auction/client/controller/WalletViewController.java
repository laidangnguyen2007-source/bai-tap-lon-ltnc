...
/**
 * Controller cho màn hình Quản Lý Ví (wallet-view.fxml). Hiển thị số dư ví và lịch sử giao dịch cho
 * Bidder/Seller.
 */
public class WalletViewController implements com.auction.client.observer.AuctionObserver {
            ...
            () -> {
              ...
              Platform.runLater(
                  () -> {
                    if (wallet != null) {
                      long available =
                          wallet.get("availableBalance") != null
                              ? ((Number) wallet.get("availableBalance")).longValue()
                              : 0;
                      long locked =
                          wallet.get("lockedBalance") != null
                              ? ((Number) wallet.get("lockedBalance")).longValue()
                              : 0;
                      long total =
                          wallet.get("totalBalance") != null
                              ? ((Number) wallet.get("totalBalance")).longValue()
                              : 0;
  ...
  private void handleRefresh(ActionEvent event) {
    ...
    NotificationUtils.showToast(
        (Stage) refreshButton.getScene().getWindow(), "✅ Đã làm mới dữ liệu ví", false);
  }
  ...
  private void handleTopUp(ActionEvent event) {
    ...
    if (amountText.isEmpty()) {
      NotificationUtils.showToast(
          (Stage) refreshButton.getScene().getWindow(), "⚠ Vui lòng nhập số tiền cần nạp!", true);
      return;
    }
    ...
    try {
      ...
      if (amount <= 0) {
        NotificationUtils.showToast(
            (Stage) refreshButton.getScene().getWindow(), "⚠ Số tiền nạp phải lớn hơn 0!", true);
        return;
      }
      ...
      alert.setContentText(
          "Bạn có chắc chắn muốn nạp " + String.format("%,d VNĐ", amount) + " vào ví không?");
      ...
      if (result.isPresent() && result.get() == javafx.scene.control.ButtonType.OK) {
        ...
        if (success) {
          ...
          NotificationUtils.showToast(
              (Stage) refreshButton.getScene().getWindow(), "❌ Nạp tiền thất bại!", true);
        }
      }
    } catch (NumberFormatException e) {
      NotificationUtils.showToast(
          (Stage) refreshButton.getScene().getWindow(), "⚠ Số tiền không hợp lệ!", true);
    }
  }
