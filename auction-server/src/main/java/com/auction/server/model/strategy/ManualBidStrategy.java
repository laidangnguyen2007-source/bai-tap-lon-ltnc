...
/**
 * Manual bidding strategy - chiến lược đấu giá thủ công
 * Người đấu giá xác định rõ số tiền đấu giá
 * Quy tắc xác thực:
 * Phiến đấu giá phải ở trạng thái đang chạy {@code RUNNING}
 * Số tiền yêu cầu phải lớn hơn số tiền đấu giá của phiên
 * Số tiền yêu cầu phải lớn hơn hoặc bằng mức giá tối thiểu của phiên
 */
  ...
  private void validateAuction(Auction auction) {
    ...
    if (auction.getStatus() != AuctionStatus.RUNNING) {
      throw new AuctionClosedException(
        "Auction #" + auction.getId() + " is not currently running (status =" + auction.getStatus() + ")."
      );
    }
  }
  ...
  private void validateAmount(Auction auction, double requestAmount) {
    ...
    if (requestAmount < currentPrice) {
      throw new InvalidBidException(
        String.format("Bid amount %.2f must be greater than current price %.2f",requestAmount,currentPrice)
      );
    }
    ...
    if (requestAmount < requiredMinimum) {
      throw new InvalidBidException(
        String.format("Bid amount %.2f doesn't meet minimum increment. Require >= %.2f.",requestAmount,requiredMinimum)
      );
    }
  } 
  @Override
